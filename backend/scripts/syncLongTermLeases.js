...
function buildTenantNote(lease) {
  const termStart = formatDate(
    lease.leaseFrom || lease.moveIn
  );

  const termEnd = formatDate(
    lease.leaseTo || lease.moveOut
  );
...
async function syncLongTermLeases() {
  ...
  console.log(`Found ${leaseMap.size} long-term unit keys`);
  ...
  console.log(`Found ${rooms.length} rooms with appfolioName`);
  ...
  for (const room of rooms) {
    ...
    const keepShortTermStatus =
      isShortTermRentedStatus(room.status);

    const newStatus = keepShortTermStatus
      ? room.status
      : "Long Term";
    ...
    if (keepShortTermStatus) {
      console.log(
        `Merged Long Term note, kept status: ${room.appfolioName} -> ${room.status}`
      );

      mergedWithShortTerm++;
    } else {
      console.log(`Updated Long Term: ${room.appfolioName}`);
  ...
  console.log(`Updated Long Term rooms: ${updatedLongTerm}`);
  console.log(
    `Merged with Airbnb/Booking rooms: ${mergedWithShortTerm}`
  );
  console.log(`Rooms without match: ${noMatch}`);

  await prisma.$disconnect();
}

syncLongTermLeases().catch(async (error) => {
